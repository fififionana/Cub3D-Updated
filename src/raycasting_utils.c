...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/06/28 17:41:22 by frushiti          #+#    #+#             */
/*   Updated: 2024/06/28 17:41:24 by frushiti         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_drawstartend(t_recup *recup)
{
	...
	if (recup->ray.side == 0)
		perpwalldist = ((double)recup->ray.mapx - recup->ray.posx + (1
					- (double)recup->ray.stepx) / 2) / recup->ray.raydirx;
	else
		perpwalldist = ((double)recup->ray.mapy - recup->ray.posy + (1
					- (double)recup->ray.stepy) / 2) / recup->ray.raydiry;
	recup->ray.perpwalldist = perpwalldist;
	recup->ray.lineheight = (int)(recup->ry / perpwalldist);
