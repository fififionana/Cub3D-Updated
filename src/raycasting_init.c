...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/06/28 17:28:03 by frushiti          #+#    #+#             */
/*   Updated: 2024/06/28 17:28:08 by frushiti         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_initialisation2(t_recup *recup)
{
	...
	rx = recup->rx;
	recup->s.zbuffer = (double *)malloc(sizeof(double) * rx);
	if (!recup->s.zbuffer)
		exit(0);
...
void	ft_initialisation3(t_recup *recup)
{
	...
	recup->ray.camerax = 2 * recup->ray.x / (double)rx - 1;
	recup->ray.raydirx = recup->ray.dirx
		+ recup->ray.planx * recup->ray.camerax;
	recup->ray.raydiry = recup->ray.diry
		+ recup->ray.plany * recup->ray.camerax;
...
void	ft_init_sprite(t_recup *recup)
{
	...
	if (!recup->sxy || !recup->s.order || !recup->s.dist)
		ft_error(recup, "Malloc error in sprite initialization\n");
	ft_init_sprite2(recup, 0, 0, v);
	ft_mlx(recup);
}
